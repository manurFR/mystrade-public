...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for trade in orm['game.Trade'].objects.all():
            offer = orm['game.Offer'].objects.create(comment = trade.comment, free_information = trade.free_information)
            for rule in trade.rules.all():
                offer.rules.add(rule)
            offer.save()
            for comm in trade.tradedcommodities_set.all():
                orm['game.TradedCommodities'].objects.create(offer = offer, commodity = comm.commodity, nb_traded_cards = comm.nb_traded_cards)
            trade.initiator_offer = offer
            trade.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        orm['game.Offer'].objects.all().delete()
        orm['game.TradedCommodities'].objects.filter(offer__isnull = False).delete()
