...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Offer.creation_date'
        db.add_column(u'trade_offer', 'creation_date',
                      self.gf('django.db.models.fields.DateTimeField')(default=datetime.datetime.now),
                      keep_default=False)

        for trade in orm['trade.Trade'].objects.all():
            trade.initiator_offer.creation_date = trade.creation_date
            trade.initiator_offer.save()
            if trade.responder_offer:
                trade.responder_offer.creation_date = trade.creation_date # not the real date, but better than now()
                trade.responder_offer.save()

    def backwards(self, orm):
        # Deleting field 'Offer.creation_date'
        db.delete_column(u'trade_offer', 'creation_date')
