        ...
        for index, victim in enumerate(players):
            if index != culprit:
                victim.register_rule('HAG06',
                              '(6) Since player #{} has {} blue cards, 10 points are deducted.'.format(culprit + 1, players[culprit].commodity('Blue')['nb_scored_cards']),
                              score = -10)
...
def HAG08(players):
    """The player with the most yellow cards gets a bonus of the number of those cards squared. 
       ...
       with the next highest number of yellows.
        ...
        # Global rulecard #
    """
    yellows = [player.commodity('Yellow')['nb_scored_cards'] for player in players]
...
def HAG12(players):
    ...
    """
    winner = None
    reds = [player.commodity('Red')['nb_scored_cards'] for player in players]
